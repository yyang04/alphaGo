...
def draw_charts(data):
    kline_data = data[['open_price', 'close_price', 'low_price', 'high_price']].values.tolist()
    data['ma'] = data['close_price'].rolling(5).mean()

    kline = (
        Kline()
        .add_xaxis(
            xaxis_data=data['dt'].dt.strftime("%Y-%m-%d").tolist()
        )
        .add_yaxis(
    ...
    line = (
        Line()
        .add_xaxis(xaxis_data=data['dt'].dt.strftime("%Y-%m-%d").tolist())
        .add_yaxis(
            series_name="MA5",
            y_axis=data['ma'].tolist(),
            is_smooth=True,
    ...
    st_pyecharts(overlap_kline_line, height="600%")
    # # bar config
    # grid_chart.add(
    #     bar,
